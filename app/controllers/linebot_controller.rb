  ...
  def callback
    ...
    events.each { |event|
      if event.message['text'] != nil
        ...
        result = `curl -X GET https://api.gnavi.co.jp/RestSearchAPI/v3/?keyid=2e1e9c94fdc118b8113077cc85fdbe97'&'format=json'&'address=#{place}`#ここでぐるなびAPIを叩く
        latitude = event.message['latitude']
        longitude = event.message['longitude']

        result = `curl -X GET https://api.gnavi.co.jp/RestSearchAPI/v3/?keyid=2e1e9c94fdc118b8113077cc85fdbe97'&'format=json'&'latitude=#{latitude}'&'longitude=#{longitude}`#ここでぐるなびAPIを叩く
      end

      hash_result = JSON.parse result #レスポンスが文字列なのでhashにパースする
      shops = hash_result["rest"] #ここでお店情報が入った配列となる
      shop = shops.sample.to_s #任意のものを一個選ぶ
